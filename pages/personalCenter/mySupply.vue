...
<template>
  <div class="myDemand-page">
    ...
    <div class="title"> {{ publish ? '发布供应信息' : '我发布的供应信息' }}</div>

    <div v-show="!publish" class="search-form">
      <el-form ref="searchForm" :model="searchForm" class="search-form-inline">
        ...
        <el-form-item label="" prop="cat3_id">
          <el-select v-model="searchForm.cat3_id" size="mini" filterable allow-create default-first-option
            placeholder="请选择分类">
        ...
        <el-form-item label="发布时间" class="time" label-width="190px">
          ...
          <!-- <el-col :span="11">
            <el-date-picker v-model="searchForm.create_time_start" type="date" placeholder="选择日期"
              value-format="yyyy-MM-dd" style="width: 100%"></el-date-picker>
          </el-col>
          <el-col class="line" :span="2">-</el-col>
          <el-col :span="11">
            <el-date-picker v-model="searchForm.create_time_stop" type="date" placeholder="选择日期"
              value-format="yyyy-MM-dd" style="width: 100%"></el-date-picker>
          </el-col> -->
        </el-form-item>
    ...
    <div v-show="!publish">
          ...
          <div class="btn-group">
            <div class="btn pub-btn" @click="publishBtn">发布供应</div>
          </div>
          <el-table :data="firsttable" tooltip-effect="dark" style="width: 100%" stripe
            ...
            <el-table-column prop="order_sn" label="单号" align="center" width="100">
            </el-table-column>
            <el-table-column prop="category_name" label="分类" align="center">
            </el-table-column>
            <el-table-column prop="number" label="采购数量" align="center" width="90">
            </el-table-column>
            <el-table-column prop="deadline" label="信息有效期" width="185" align="center">
            ...
            <el-table-column prop="release_time" label="操作" align="center" width="140">
              <template slot-scope="scope">
                <el-popconfirm :title="`确定删除？`" hide-icon confirm-button-text="取消" cancel-button-text="删除"
                  popper-class="popper-delete" @cancel="deleteDemand(scope.row.id)">
                  <el-button v-if="scope.row.status == 1 || scope.row.status == 3" slot="reference" size="mini"
                ...
                <el-button v-if="scope.row.status == 2" size="mini" type="text" icon="el-icon-circle-close"
                  @click="closeDemand(scope.row.id)">关闭</el-button>
                <el-button v-if="scope.row.status == 4" size="mini" type="text"
                  @click="reUse(scope.row.id)">重新激活</el-button>
                <el-button slot="reference" size="mini" type="text" icon="el-icon-view"
                  @click="editDemand(scope.row.id)">查看</el-button>
              </template>
            </el-table-column>
          </el-table>
    ...
    <div v-if="publish">
      ...
      <publishComponent :showBindGoods="true" v-if="searchForm.type != 3" :id="orderId" :type="searchForm.type"
        :information_type="searchForm.information_type" @publishSuccess="publishSuccess" @publishDelete="publishDelete"
        @publishBack="publishBack" @publishClose="publishClose"></publishComponent>
      <publishCarComponent v-else :id="orderId" :is-supply="searchForm.information_type" :type="searchForm.type"
        :information_type="searchForm.information_type" @publishSuccess="publishSuccess" @publishDelete="publishDelete"
        @publishBack="publishBack" @publishClose="publishClose">
      </publishCarComponent>
    </div>
...
export default {
  ...
  methods: {
    ...
    getsupplyOrderList() {
      ...
      this.$request
        .get(Api.getUserSupplyOrderList, { params })
        .then((res) => {
