<template>
  <div class="wrap">
    <div class="content">
      <div class="settlement-page">
        ...
        <el-collapse v-model="receiveNames">
          <el-collapse-item title="收货人信息" name="1">
            ...
            <div class="receive-info-container">
              <div class="receive-info" :class="addressSelected.id === address.id ? 'active' : ''"
                v-for="(address, i) in addressList" :key="i" @click="selectAddress(i, address)">
                <div class="text-ellipsis-2">{{ address.pcd_address }}</div>
                <div class="name-tel">
                  <div>{{ address.consignee }}</div>
                  <div>{{ address.mobile }}</div>
                </div>
                <div class="edit-btn" @click.stop="editAddress(address)">
                  修改>
                </div>
              </div>
        ...
        <el-collapse v-model="invoiceNames">
          <el-collapse-item title="发票信息" name="1" disabled>
            <template slot="title">
              <div>
                <span>发票信息</span>
                <el-checkbox v-model="is_open_invoice" :true-label="1" :false-label="0" style="margin-left: 40px"
                  @change="openInvoice">开发票
                </el-checkbox>
              </div>
            ...
            <div class="invoice-info-container">
              <div class="invoice-info" :class="billHeaderSelected.id === billHeader.id ? 'active' : ''"
                v-for="(billHeader, i) in billHeaderList" :key="i" @click="selectbillHeader(i, billHeader)">
                <div class="type">{{ billHeader.inv_type_name }}</div>
                <div class="text-ellipsis-2">{{ billHeader.title }}</div>
                <!-- <div>{{ billHeader.address }}</div> -->
                <div class="edit-btn" @click.stop="editInvoiceEdit(billHeader)">
                  修改>
                </div>
              </div>
              <div class="btn">
        ...
        <el-collapse v-model="productNames">
          <el-collapse-item title="产品信息" name="1" disabled>
            <div v-for="(store, i) in orderProductList" :key="i" class="store-item">
              <div class="header">
                <div class="store">
                  店铺：<span @click="gotoStoreDetails(store.shop_id)">{{
                    store.shop_name
                  }}</span>
                </div>
                <div>
                  商品金额：<span class="price">￥{{ store.goods_amount }}</span>
                </div>
                <div>
                  运费：<span class="price">{{
              ...
              <div class="product-list">
                <div class="left">
                  <div v-for="pro in store.children" :key="pro.id" class="row" :class="addressSelected&&addressSelected.pcd &&
                      pro.market_area_name.includes(addressSelected&&addressSelected.pcd[0])
                      ? ''
                      : 'gray_row'
                    ">
                    <img :src="pro.image" alt="" style="width: 64px; height: 64px" @click="gotoDetails(pro.sku_id)" />
                    <div class="summary" @click="gotoDetails(pro.sku_id)">
                      <div class="desc">{{ pro.goods_name }}</div>
                      <div class="type">{{ pro.attr_name }}</div>
                    </div>
                    <div class="price-single" v-if="
                      addressSelected&&addressSelected.pcd &&
                      pro.market_area_name.includes(addressSelected&&addressSelected.pcd[0])
                    ">
                      ￥{{ pro.sku_shop_price }}
                    </div>
                    <div class="count" v-if="
                      addressSelected&&addressSelected.pcd &&
                      pro.market_area_name.includes(addressSelected&&addressSelected.pcd[0])
                    ">
                      x
                    ...
                    <div style="text-align: center; line-height: 7; width: 300px" v-if="
                      addressSelected.pcd &&
                      !pro.market_area_name.includes(addressSelected&&addressSelected.pcd[0])
                    ">
                      商品不支持配送该区域
                ...
                <div class="right">
                  <el-input type="textarea" v-model="store.postscript" placeholder="请输入内容">
                  </el-input>
                </div>
              </div>
            </div>
        ...
        <div class="bottom-btn-group">
          ...
          <el-tooltip class="item" effect="dark" content="请提交公司认证信息" placement="top-start" v-if="
            userInfo.status == 3 ||
            userInfo.status == 4 ||
            userInfo.status == 5
          ">
            <div class="submit-order-btn-disable" @click="submitOrder">
              提交订单
            </div>
          </el-tooltip>
          <div v-else class="submit-order-btn" @click="submitOrder">
            提交订单
          </div>
        </div>
      </div>
    ...
    <!-- 弹框 -->
    <!-- 新增地址弹框 -->
    <el-dialog title="新增收货地址" :visible.sync="newAddressFormVisible" width="540px" center class="add-new-address-dialog">
      <el-form ref="newAddressForm" :model="newAddressForm" :rules="newAddressFormRules" label-width="100px">
        <el-form-item label="所在地区" prop="region">
          <el-cascader style="width: 100%" v-model="newAddressForm.region" :options="options"
    ...
    <!-- 新增发票抬头 -->
    <el-dialog title="新增发票抬头" :visible.sync="newBillHeaderFormVisible" width="540px" center
      class="add-new-billheader-dialog">
      <el-form ref="newBillHeaderForm" :model="newBillHeaderForm" :rules="newBillHeaderFormRules" label-width="80px">
    ...
    <!-- 选择其他地址 -->
    <el-dialog title="选择收货地址" :visible.sync="otherAddressTableVisible" width="860px" center
      class="other-address-dialog">
      <el-table :data="addressTableData" @row-click="clickRowAddress" style="width: 100%" stripe
        :header-row-style="{ background: 'rgba(137,147,165,0.06)' }" height="450" :header-cell-style="{
          background: 'transparent',
          color: '#353535',
          fontSize: '14px',
        }" align="center">
        <el-table-column prop="pcd_address" label="收货地址" align="center">
        </el-table-column>
        <el-table-column prop="consignee" label="收货人" align="center" width="120px">
        </el-table-column>
        <el-table-column prop="mobile" label="联系电话" align="center" width="150px">
        </el-table-column>
        ...
        <el-table-column fixed="right" label="操作" width="180" align="center">
          <template slot-scope="scope">
            <el-button type="text" size="small" style="color: #353535"
              @click.native.prevent.stop="editAddress(scope.row)">
              修改
            </el-button>
            <el-popconfirm :title="`确定删除？`" hide-icon confirm-button-text="取消" cancel-button-text="删除"
              popper-class="popper-delete" @cancel="deleteAddress(scope.row.id)">
              <el-button slot="reference" @click.stop size="mini" type="text" style="color: #353535">删除</el-button>
            </el-popconfirm>
            <el-button v-if="scope.row.is_default === 0" type="text" size="small"
              @click.native.prevent.stop="setDefaultAddress(scope.row)" style="color: #353535">
              设为默认
            </el-button>
          </template>
        </el-table-column>
    ...
    <!-- 选择其他发票抬头 -->
    <el-dialog title="选择发票抬头" :visible.sync="otherBillHeaderTableVisible" width="660px" center
      class="other-billheader-dialog">
      <el-table :data="billHeaderTableData" @row-click="clickRowBillHeader" style="width: 990px" stripe
        :header-row-style="{ background: 'rgba(137,147,165,0.06)' }" height="450" :header-cell-style="{
          background: 'transparent',
          color: '#353535',
          fontSize: '14px',
        }" align="center">
        <el-table-column prop="inv_type_name" label="发票类型" align="center">
        </el-table-column>
        <el-table-column prop="is_company_name" label="类型" align="center" width="80px">
        </el-table-column>
        ...
        <el-table-column fixed="right" label="操作" width="130" align="center">
          <template slot-scope="scope">
            ...
            <el-popconfirm :title="`确定删除？`" hide-icon confirm-button-text="取消" cancel-button-text="删除"
              popper-class="popper-delete" @cancel="deleteInvoice(scope.row.id)">
              <el-button slot="reference" size="mini" @click.stop type="text" style="color: #353535">删除</el-button>
            </el-popconfirm>
          </template>
        </el-table-column>
      </el-table>
...
export default {
  ...
  async mounted() {
    ...
    if (this.$route.query.sku_id) {
      this.number = JSON.parse(this.$route.query.number);
      // console.log('this.$route.query.sku_id', typeof this.$route.query.sku_id);
      if (typeof this.$route.query.sku_id === "string") {
        this.sku_id = [this.$route.query.sku_id * 1];
      } else {
        this.sku_id = this.$route.query.sku_id.map((id) => id * 1);
      }
  ...
  methods: {
    ...
    getPreOrder() {
      ...
      this.$request.post(Api.postPreOrder, data).then((res) => {
        if (res.code === 0) {
